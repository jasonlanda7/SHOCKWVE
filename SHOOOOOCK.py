...
def rao_bell_nozzle(
    ...
    # ===== FORCE connection to chamber radius (Convention A) =====
    if use_cea:
      ...
      if include_chamber:
        if use_cea:
            ...
            # DEFINE A TRUE CHAMBER (CYLINDER)
            # Non-CEA Rao: visualization-only chamber
            Rc_use = 2.5 * Rt
            Lc_use = 2.0 * Rt  # short, MOC-like scale
        ...
        # Chamber axial coordinates (upstream)
        # --- Smooth chamber → convergent blend (C¹ continuous) ---
            ...
            if self.nozzle_mode_cb.isChecked():
                ...
                if self.cea_mode_cb.isChecked():
                    try:
                        ...
                        Lstar_m = float(self.Lstar_edit.text()) / 10
